...
def get_leading_score(event_name, news_time, calendar):
    if "Farm" in event_name:
        ...
        return calc_score(adp)*1.5 + calc_score(claims, True)*1.0
...
def main():
    ...
    print("\n=== BRUTE-FORCE PREDICTION OPTIMIZER ===")
    print("Searching for an algorithm that yields > 70% Win Rate...\n")
    
    # Define models (lambda functions that take a row and return 1 (BUY), -1 (SELL), or 0 (SKIP))
    models = {
        "M1: Naive Forecast": lambda r: r["Forecast"],
        "M2: Inverse Forecast (Buy the Rumor)": lambda r: -r["Forecast"],
        "M3: Follow 1H Trend (Institutional Drift)": lambda r: r["Trend_1h"],
        "M4: Inverse 1H Trend (Mean Reversion)": lambda r: -r["Trend_1h"],
        "M5: Follow 4H Trend": lambda r: r["Trend_4h"],
        "M6: Leading Indicators Only": lambda r: 1 if r["Leading"] >= 1 else (-1 if r["Leading"] <= -1 else 0),
        "M7: Forecast + 1H Trend Match": lambda r: r["Forecast"] if (r["Forecast"] == r["Trend_1h"] and r["Forecast"] != 0) else 0,
        "M8: Forecast + 4H Trend Match": lambda r: r["Forecast"] if (r["Forecast"] == r["Trend_4h"] and r["Forecast"] != 0) else 0,
        "M9: Mean Revert 15m Spike": lambda r: -r["Trend_15m"],
    }
    ...
    for group in events_to_test.keys():
        ...
        for name, model_func in models.items():
            ...
            trades = wins + losses
            if trades < 3: continue # Ignore models that skip almost everything
        ...
        # Sort and print all models that hit >= 65%
        viable = [r for r in results_for_group if r[1] >= 0.65]
        viable.sort(key=lambda x: x[1], reverse=True)
        
        if viable:
            print(f"SUCCESS: Found {len(viable)} models > 65% Win Rate for {group}:")
            for v in viable:
                print(f"   -> {v[1]*100:.1f}% ({v[2]}/{v[3]} trades) | {v[0]}")
        else:
            print(f"FAIL: No single model achieved > 65% for {group}. Best was:")
