...
def main():
    ...
    stats_by_event = {g: {"wins": 0, "losses": 0, "usd": 0.0} for g in events_to_test.keys()}
    total_pot = 5000.0
    ...
    for ev in all_events:
        ...
        if net > 0: stats_by_event[ev['group']]["wins"] += 1
        else: stats_by_event[ev['group']]["losses"] += 1
            
        stats_by_event[ev['group']]["usd"] += net
        total_pot += net
        
    print("EVENT           | TOTAL | WINS  | LOSSES | WIN RATE | TOTAL PRODUCED (USD)")
    print("-" * 76)
    
    # Sort for printing by win rate
    sorted_events = sorted(stats_by_event.items(), key=lambda x: x[1]["wins"] / max(1, (x[1]["wins"]+x[1]["losses"])), reverse=True)
    
    for event, stats in sorted_events:
        total = stats["wins"] + stats["losses"]
        if total == 0: continue
        win_rate = (stats["wins"] / total) * 100
        print(f"{event:<15} | {total:<5} | {stats['wins']:<5} | {stats['losses']:<6} | {win_rate:>5.1f}%   | ${stats['usd']:>15,.2f}")
        
    print("-" * 76)
    print(f"FINAL COMPOUNDED POT (No PCE): ${total_pot:,.2f}")
