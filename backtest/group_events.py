...
def main():
    ...
    # Run the simulation but store results directly in a list of dicts for grouping
    results_grouped = defaultdict(list)
    stats_by_event = defaultdict(lambda: {"wins": 0, "losses": 0})
    ...
    for ev in all_events:
        ...
        if net > 0:
            stats_by_event[ev['group']]["wins"] += 1
        else:
            stats_by_event[ev['group']]["losses"] += 1
            
        total_pot += net
        
        dt_str = str(ev['time'])[:16]
        # We store the exact string output per trade for the event breakdown
        # I won't print the changing total_pot in the grouped view since jumping out of chronological order makes reading pot balances confusing.
        line_str = f"{dt_str} | Acct A (Bias): ${pnl_a:11,.2f} | Acct B (Hedge): ${pnl_b:11,.2f} | Net Walkaway: ${net:11,.2f}"
        results_grouped[ev['group']].append(line_str)
    ...
    for event, stats in stats_by_event.items():
        ...
        print(f"{event:<15} | {total:<5} | {stats['wins']:<5} | {stats['losses']:<6} | {win_rate:.1f}%")
        
    print("\n")
    print("=== EXACT TRADES SORTED BY EVENT ===")
    for event, lines in results_grouped.items():
        print(f"\n--- {event.upper()} ---")
        for line in lines:
            print(line)
