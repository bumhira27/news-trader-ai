...
def main():
    ...
    print("DATE & EVENT             | AI BIAS | NET WALKAWAY   | TOTAL POT")
    print("-" * 75)
    ...
    for ev in all_events:
        ...
        # Adding marker if we just crossed 10k
        if total_pot >= 10000.0 and (total_pot - net) < 10000.0:
            print("-" * 75)
            print("*** POT CROSSED $10,000. COMPOUNDING BEGAN ***")
            print("-" * 75)

        print(f"{dt_str} | {ev['group']:<12} | {ev['adv_bias']:<7} | Net: ${net:10,.2f} | Pot: ${total_pot:11,.2f}")
        
    print("-" * 75)
    print(f"FINAL COMPOUNDED POT: ${total_pot:,.2f}")
